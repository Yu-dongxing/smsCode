...
- 回补规则: 保存供应商配置时允许 `{{变量}}` URL 模板；运行请求时仍校验变量替换后的真实 URL。
- 开发状态: CODE_DONE
- 测试状态: TEST_PASS
- 验收状态: WAIT_ACCEPT
...
- 模板变量修复验证结果: `sh mvnw -q -Dtest=UrlSecurityUtilTest test` 通过；`sh mvnw -q -DskipTests compile` 通过。
...
时间: 2026-07-08
修改人: Codex
修改原因: 修复保存供应商配置时 `{{变量}}` 导致 URL 校验误报。
修改内容: 保存配置阶段允许 URL 模板变量，运行请求阶段保持最终 URL 校验。
影响范围: 项目配置保存、供应商 URL 校验。
关联 ID: REQ-SEC-20260708-001, REQ-SEC-20260708-002
